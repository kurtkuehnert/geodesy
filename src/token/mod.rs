...
pub fn parse_proj(definition: &str) -> Result<String, Error> {
    ...
    for (step_index, step) in steps.iter().enumerate() {
        ...
        if !geodesy_step.is_empty() {
            ...
            let axis_step = extract_axis_step(&mut elements)?;
...
fn normalize_omerc(elements: &mut Vec<String>) {
    ...
    let no_uoff = remove_parameter_flag(elements, "no_uoff") || remove_parameter_flag(elements, "no_off");
...
fn extract_axis_step(elements: &mut Vec<String>) -> Result<Option<String>, Error> {
