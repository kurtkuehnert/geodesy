...
fn inv(op: &Op, _ctx: &dyn Context, operands: &mut dyn CoordinateSet) -> usize {
    ...
    for i in 0..operands.len() {
        ...
        let lon = if sp.abs() <= TOL {
            ...
            let arg = x * lat.tan() * (1.0 - es * sp * sp).sqrt();
            arg.clamp(-1.0, 1.0).asin() / sp
        } + lon_0;
...
mod tests {
    ...
    fn poly_inverse_matches_proj_world_polyconic() -> Result<(), Error> {
        ...
        let mut projected = [Coor4D::raw(3_122_659.2518, 2_492_720.8888, 0.0, 0.0)];
    ...
    fn poly_inverse_matches_proj_brazil_polyconic_point() -> Result<(), Error> {
        ...
        let mut projected = [Coor4D::raw(5_438_546.7142, 8_891_486.8987, 0.0, 0.0)];
        ctx.apply(op, Inv, &mut projected)?;
